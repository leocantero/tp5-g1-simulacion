...
# =========================
ordenes = pd.read_csv(DATA_DIR / "olist_orders_dataset.csv")
...
ordenes["order_purchase_date"] = pd.to_datetime(
    ordenes["order_purchase_timestamp"]
).dt.normalize()
...
percentiles = ordenes_2017["order_total"].quantile(
...
ordenes_2017_filtrado = (
    ordenes_2017[ordenes_2017["order_total"] <= p95]
    .copy()
)
...
for col in columnas_fecha:
    ordenes_2017_filtrado[col] = pd.to_datetime(
        ordenes_2017_filtrado[col],
        errors="coerce"
    )
...
ordenes_ia = (
    ordenes_2017_filtrado[
        ordenes_2017_filtrado["IA_min"] <= umbral_ia
    ]
    .copy()
)
...
# =========================
muestra_q1 = ordenes_ia[
    ordenes_ia["order_purchase_timestamp"].dt.month.isin([1, 2])
]

muestra_q2 = ordenes_ia[
    ordenes_ia["order_purchase_timestamp"].dt.month.isin([6, 7])
]

muestra_q4 = ordenes_ia[
    ordenes_ia["order_purchase_timestamp"].dt.month.isin([11, 12])
]
...
ordenes_ia["TA_horas"] = (
    ordenes_ia["order_delivered_carrier_date"]
    - ordenes_ia["order_approved_at"]
...
ordenes_ta_filtrado = (
    ordenes_ia[ordenes_ia["TA_horas"] <= p95_ta]
    .copy()
)
...
# =========================
muestra_q1 = ordenes_ta_filtrado[
    ordenes_ta_filtrado["order_purchase_timestamp"].dt.month.isin([1, 2])
]

muestra_q2 = ordenes_ta_filtrado[
    ordenes_ta_filtrado["order_purchase_timestamp"].dt.month.isin([6, 7])
]

muestra_q4 = ordenes_ta_filtrado[
    ordenes_ta_filtrado["order_purchase_timestamp"].dt.month.isin([11, 12])
]
...
print(
    "Enero-Febrero   | Normal:",
    muestra_q1_normal.shape,
    "| Alta:",
    muestra_q1_alta.shape
)

print(
    "Junio-Julio     | Normal:",
    muestra_q2_normal.shape,
    "| Alta:",
    muestra_q2_alta.shape
)

print(
    "Noviembre-Dic   | Normal:",
    muestra_q4_normal.shape,
    "| Alta:",
    muestra_q4_alta.shape
)
...
resultados_fdp["TEA_ene-feb"] = ajustar_y_reportar(
    "TEA ene-feb (Alta)",
    muestra_q1_alta["TA_horas"]
)

resultados_fdp["TEN_ene-feb"] = ajustar_y_reportar(
    "TEN ene-feb (Normal)",
    muestra_q1_normal["TA_horas"]
)

resultados_fdp["TEA_jun-jul"] = ajustar_y_reportar(
    "TEA jun-jul (Alta)",
    muestra_q2_alta["TA_horas"]
)

resultados_fdp["TEN_jun-jul"] = ajustar_y_reportar(
    "TEN jun-jul (Normal)",
    muestra_q2_normal["TA_horas"]
)

resultados_fdp["TEA_nov-dic"] = ajustar_y_reportar(
    "TEA nov-dic (Alta)",
    muestra_q4_alta["TA_horas"]
)

resultados_fdp["TEN_nov-dic"] = ajustar_y_reportar(
    "TEN nov-dic (Normal)",
    muestra_q4_normal["TA_horas"]
)
